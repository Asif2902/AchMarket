      ...
      try {
        ...
        const results = await Promise.allSettled(
          resolvedSummaries.map(async (s: { market: string; title: string }) => {
            ...
            const [resolvedPoolWei, feePoolWei] = await Promise.all([
              marketContract.resolvedPoolWei(),
              marketContract.feePoolWei().catch(() => 0n),
            ]);
            return { s, resolvedPoolWei, feePoolWei };
          })
        );
        ...
        for (const result of results) {
          ...
          const { s, resolvedPoolWei, feePoolWei } = result.value;
          resolvedCount++;

          if (resolvedPoolWei > 0n) {
            const fee = feePoolWei;
            feesSum += fee;
            resolvedVol += resolvedPoolWei;
            events.push({
              market: s.market,
              title: s.title,
              amount: fee,
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="card p-5 text-center">
          ...
          <p className="text-2xl sm:text-3xl font-bold text-primary-400 tabular-nums">Tracked</p>
          <p className="text-xs text-dark-400 mt-1">Fee Pool</p>
        </div>
        <div className="card p-5 text-center">
          ...
          <p className="text-2xl sm:text-3xl font-bold text-white tabular-nums">{formatCompactUSDC(totalFeesCollected)}</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/[0.08] text-dark-400 text-left">
                  ...
                  <th className="pb-3 font-medium text-xs uppercase tracking-wider text-right">Tracked Fees</th>
                </tr>
              </thead>
              <tbody>
                {feeEvents.map((evt, i) => (
                  <tr key={i} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="py-3.5 pr-4 text-right">
                      <span className="text-white font-semibold tabular-nums">{formatCompactUSDC(evt.resolvedPoolWei)}</span>
                    </td>
                    <td className="py-3.5 text-right">
                      <span className="text-emerald-400 font-semibold tabular-nums">{formatCompactUSDC(evt.amount)}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {totalResolved > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="card p-5">
            ...
            <p className="text-xl font-bold text-white tabular-nums">{formatCompactUSDC(totalResolvedVolume)} USDC</p>
          </div>
          <div className="card p-5">
            <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-xl font-bold text-white tabular-nums">
              {totalResolved > 0 ? formatCompactUSDC(totalFeesCollected / BigInt(totalResolved)) : '0'} USDC
            </p>
          </div>
        </div>
      )}
